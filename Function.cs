...
namespace QL_Vaccine.DAO
{
    public class Function
    {
        ...
        {
            get { if (instance == null) instance = new Function(); return instance; }
            private set { instance = value; }
        }

        private Function() { }

        public void tachHoTen(string hoTen, out string ho, out string tenDem, out string ten)
        {
            var chuoiTen = hoTen.Trim().Split(' ');
            ho = chuoiTen[0];
            ten = chuoiTen[chuoiTen.Length - 1];
            tenDem = "";
            for (int i = 1; i < chuoiTen.Length - 1; i++)
            {
                tenDem += chuoiTen[i] + " ";
            }
            tenDem = tenDem.Trim();
        ...
        public bool checkTextBoxNullOrDoubleType(string text)
        {
            if (text == "" || !(double.TryParse(text, out double result)))
                return true;
            return false;
        }
